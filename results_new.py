...
print("RD-GA performance: " + str(mean_ga_hist))
...
sns_plot = sns.histplot(data=df, x="Epoch", hue = "Objective")
...
# compare the RD-GA-40, and the SD-GA objs mean oos TE
data_GA = dict([])
data_GA["TE"] = []
data_GA["rebalance date"] = []
data_GA["Model"] = []
...
sns.lineplot(x="rebalance date", y="TE", hue="Model", data = df_1, ax = axs[0])
sns.lineplot(x = 'Date', y = 'Cumulative Return', hue="Model", data = df_2, ax = axs[1])
...
sns.set_theme(style="ticks", font_scale=1.4)
g = sns.FacetGrid(data = df_after_plot, col="Objective")
