...
def GQ_MiLS(model, r, k, T, num_executions, target_sol):
    ...
    RCL_size = round(0.45*(r.shape[0]-1))
    pMiLS = 0.4
    UB_MiLS = round(0.15*RCL_size) 
    phi = 3

    for run in range(num_executions): 

        print("execution = " + str(run+1) + " - GQ-MiLS")
        ...
        while abs(t0-t1)<timelimit and nit < max_iter:
            ...
            # MiLS - not getting inside the loop
            if random.uniform(0,1) <= pMiLS:
                swapQuant = phi
                while swapQuant > 0:
                    S_mils, fval_mils, Scmils = MiLS(UB_MiLS, S, S_c, f_val_temp, selected_solver, t0, t1, timelimit, r, swapQuant)
            ...
            if f_val_temp < current_bestfi:
                bestfi = f_val_temp
                S_final = [element for element in S]
                S_c_final = [element for element in S_c]
