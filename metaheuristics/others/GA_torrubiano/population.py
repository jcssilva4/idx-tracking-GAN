...
class Population:
    def __init__(self,model,popsize,card,num_mutation,r,T):
        ...
        self.Solver=solver.Solve(T, r)
        self.id=self.Solver.get_id()    #1 for wang,0 for torrubiano
        self.insample=T
        self.outsample=290-T
        if self.id==0:
    ...
    def crossover(self,parent1,parent2):           #this function performs the crossover method in Torrubiano2009 an return the childs in Torrubiano encoding
    ...
    def get_mse(self,comp_vector): #calculates the mse to a given composition vector
        ...
        if len(comp_vector)==self.lenght:
            ...
            mse=0
            for t in range(self.insample): #for this porpuse, we consider only the in-sample period
                sum=0
                for j in range(self.lenght-1):
                    sum+=comp_vector[j]*r[j+1,t]
                err=(sum-r[0,t])**2
                mse+=err
            mse=mse/self.insample
            return mse
    ...
    def bin(self,parent):
        if len(parent)==self.lenght:
            print("Parent already uses the binary encode, dont need to call this method again.")
            return -1
        else:
    ...
    def rar(self,parent1,parent2,w):
        parent1,paren2=parent1.copy(),parent2.copy()
        if len(parent1)==self.lenght or len(parent2)==self.lenght:
            print("One or more of the given parent uses the binary encoding. Fix this before usnig the  population.rar() method.")
            return -1
        A,B,C,D,E,G=[],[],[],[],[],[]
        parent1,parent2=parent1.copy(),parent2.copy()
        set1,set2,uset=set(parent1),set(parent2),set([i for i in range(1,self.lenght+1)])
        A=list(set1&set2)
        sA=set(A)
        B=list(uset-set1-set2)
        sB=set(B)
        C=list(set1-set2)
        sC=set(C)
        D=list(set2-set1)
        ...
        while len(child)!=self.k:
            element=G.pop(rand.randint(0,len(G)-1))
            if element in sA|sC:
