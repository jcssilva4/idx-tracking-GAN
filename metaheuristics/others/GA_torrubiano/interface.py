...
for k in kvalues:
    ...
    id=pop.get_id()
    mod=model.Model(k,pop.lenght,id,ibov)
    ...
    relfit,bfitness,parents,compositions,iterations=[],[],[],[],[]
    count=0     #number of iteration that gone optimal
    os.chdir("..\solutions")
    if ibov==False:
        ar=open("t_"+str(filename[:-4])+"_"+str(k)+"_"+str(timelimit)+"_results_"+cplex[0]+".txt","w") #[:-4] is to avoid the '.txt' in filename
    if ibov==True:
        ar=open("t_ibov_"+str(k)+"_"+str(timelimit)+"_results_"+cplex[0]+".txt","w")
        ...
        while abs(t0-t1)<timelimit:
            ...
            for i in range(len(population)):
                if check(fitness[i],optmse)==1:
                    ...
                    compositions.append(pop.fit(pop.bin(population[bi]),id,1))
                    c=1
                    break
            if c==1:
                print("CONVERGENCE!")
