...
import time
#v5.0- make the process store worst and best compositions and parents of the 30 instances
k=5
popsize=15
filename="indtrack2.txt"
num_mutation=1
pop=populationn.Population(filename,popsize,k,num_mutation)
mod=model.Model(k,pop.lenght)
...
parents,compositions=[],[]

count=0     #number of iteration that gone optimal
...
def check(element1,element2):
    ...
    if element1==element2:
        ...
        return 0
os.chdir("..\solutions")
ar=open(str(filename[:-4])+"_"+str(k)+"_"+str(timelimit)+"_results.txt","w") #[:-4] is to avoid the '.txt' in filename
for ro in range(it):
    pop.gen_pop()
    population=pop.population.copy()    ##
    fitness=pop.fit_all(population)
    print("------------------------------------------------------------------------------------------------------------------")
    t0=time.time()
    t1=time.time()
    c,dontrepeat=0,0
    while abs(t0-t1)<timelimit:
        ...
        while checkl(child1,population)!=0 or checkl(child2,population)!=0:#remove children already included
            ...
            fitchild1,fitchild2=pop.fit(binchild1),pop.fit(binchild2)
...
ar.write("Filename: %s|Time limit: %d|Mutation Rate: %d|k=%d|Population Size: %d|Objective Value: %.12f\n"%(filename,timelimit,mutation_rate,k,popsize,optmse))

ar.close()
print("File created: "+str(filename[:-4])+"_"+str(k)+"_"+str(timelimit)+"_results.txt")
